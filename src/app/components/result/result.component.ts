...
export class ResultComponent implements OnInit, OnChanges {
    ...
    constructor(
        ...
        this.directAccess =
            this.activatedRoute.snapshot.data[0] === undefined
                ? false
    ...
    private displayResult(domainCheckId: string, language: string) {
        this.dnsCheckService
            .getTestResults({ id: domainCheckId, language })
            .then(
                (data) => {
                    // TODO clean
                    ...
                    for (const item of data['results']) {
                        this.level_items[item['level'].toLowerCase()].push(
                            item
                        );
                    }
    ...
    public getHistory() {
        if (!this.history) {
            this.alertService.info(
                'History information request is in progress'
            );
            this.dnsCheckService
                .getTestHistory(this.historyQuery)
                .then((data) => {
    ...
    public exportHTML() {
        ...
        setTimeout(() => {
            ...
            clone
                .querySelectorAll('.result > div.row > div.col-md-6')[1]
                .remove();
    ...
    private setItemsColors(data): void {
        for (const item in data) {
            if (['WARNING'].includes(this.result[item].level)) {
                ...
                ['ERROR', 'CRITICAL'].includes(this.result[item].level)
            ) {
    ...
    public setModulesColors(result): void {
        this.modules = result.reduce((modules, item) => {
            ...
            const moduleLevel = this.levelSeverity.indexOf(
                modules[item.module]
            );
    ...
    public togglePillFilter(name) {
        ...
        const atLeastOneActive = Object.keys(this.result_filter)
            .slice(1, -1)
            .filter((el) => this.result_filter[el]);
