...
describe('Zonemaster test FR03 - [All appropriate fields should be writable]', () => {
    ...
    it('should be able to write in the main input', () => {
        ...
        expect(
            element(by.css('#domain_check_name')).getAttribute('value')
        ).toBe('afnic.fr');
        ...
        expect(
            element(by.css('input[name="form.ns"]')).getAttribute('value')
        ).toBe('afnic.fr');

        element(by.css('input[name="form.keytag"]')).sendKeys('afnic.fr');
        expect(
            element(by.css('input[name="form.keytag"]')).getAttribute('value')
        ).toBe('afnic.fr');
    });
});
