...
describe('Zonemaster test FR24 - [The list of previous runs should contain links to the previous tests]', () => {
    ...
    it('should display previous run link', async () => {
        ...
        await browser.wait(
            () => element(by.css('a.btn.history')).isPresent(),
            120 * 1000
        );

        await expect(element(by.css('a.btn.history')).getText()).toEqual(
            'History'
        );
        ...
        await expect(
            element
                .all(
                    by.css(
                        '.list-group-item.list-group-item-action.list-group-item-success > a'
                    )
                )
                .get(0)
                .getAttribute('href')
        ).toEqual('http://localhost:4201/result/84bfac6ae74d0e62');
        await expect(
            element
                .all(
                    by.css(
                        '.list-group-item.list-group-item-action.list-group-item-danger > a'
                    )
                )
                .get(0)
                .getAttribute('href')
        ).toEqual('http://localhost:4201/result/293f626579274f18');
    });
});
