...
describe('Zonemaster test FR20 - [The user must be able to submit one or more DS record(s) for use with DNSSEC]', () => {
  ...
  it('should display progress bar when we add a NS ip and launch a test',  async() => {
    ...
    await element(by.css('input[formControlName="digest"]')).sendKeys('d2681e301f632bd76544e6d5b6631a12d97b5479ff07cd24efecd19203c77db3');
    await element(by.cssContainingText('select[formControlName="algorithm"] > option', 'RSASHA256')).click();
    await element(by.cssContainingText('select[formControlName="digtype"] > option', 'SHA-256')).click();
