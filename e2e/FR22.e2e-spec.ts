...
describe(
    'Zonemaster test FR22 - [Provide the possibility to see more information about encountered errors ' +
    ...
    () => {
        ...
        it('should display full messages', async () => {
            ...
            await browser.wait(
                () => element(by.css('#collapsed_module_1')).isPresent(),
                120 * 1000
            );

            await expect(
                element(by.css('#collapsed_module_1')).getText()
            ).toEqual('BASIC');
            await expect(
                element.all(by.css('#collapsed_module_1 > tr.show')).count()
            ).toBe(0);
            await element(
                by.css('#collapsed_module_1 > tr:nth-child(1)')
            ).click();
            await expect(
                element.all(by.css('#collapsed_module_1 > tr.show')).count()
            ).toEqual(14);
