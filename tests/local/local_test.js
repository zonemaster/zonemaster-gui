...
module.exports = {
    Navigation: function (browser) {
        ...
        browser
            ...
            .elTextIs(
                `.container-fluid > :nth-child(1) h4`,
                `CSS`,
                `About Zonemaster`,
                `El text is...`
            )
            ...
            .click(
                `div > div > div > div > a:nth-of-type(2)`,
                `CSS`,
                `Click element`
            )
            ...
            .elTextIs(
                `.alert`,
                `CSS`,
                `Notice! More info on undelegated test`,
                `El text is...`
            )
            .click(
                `.navbar-nav > :nth-child(2) .nav-link`,
            ...
            .pause(2000)
            .inputValueAssert(
                `div > div:nth-of-type(2) > div > div > form > div > div:nth-of-type(5) > input`,
                `CSS`,
                `ns3.nic.fr`,
                `Input value is...`
            )
            .click(
                `.ng-pristine > :nth-child(5) .fa-trash`,
                `CSS`,
                `Click element`
            )
            .click(
                `.ng-pristine > :nth-child(4) .fa-trash`,
                `CSS`,
                `Click element`
            )
            .click(
                `div:nth-of-type(2) > div > div > form > div > div:nth-of-type(3) > button > i`,
                `CSS`,
                `Click element`
            )
            .click(
                `.ng-pristine > :nth-child(2) .fa-trash`,
                `CSS`,
                `Click element`
            )
            .elementPresent(
                `app-form > div > div:nth-of-type(2) > div > div > form`,
                `CSS`,
                `El is present`
            )
            ...
            .elTextIs(
                `.alert-danger`,
                `CSS`,
                `Choisissez au moins un protocole.`,
                `El text is...`
            )
            .click(`#protocol_ipv6`, `CSS`, `Click element`)
            ...
            .pause(1000)
            .elTextIs(
                `app-result > div > div > div:nth-of-type(2) > div:nth-of-type(2) > span`,
                `CSS`,
                `IPv4`,
                `El text is...`,
                15000
...
function bindHelpers(browser) {
    ...
    function prepStringFuncForExecute(funcToExecute) {
        return (
            'var passedArgs = Array.prototype.slice.call(arguments,0); return ' +
            funcToExecute +
                ...
                function (result) {
                    if (
                        ...
                        (pathname instanceof RegExp
                            ? pathname.test(result.value.pathname)
                            : result.value.pathname === pathname)
                    ) {
    ...
    browser.scrollWindow = function (x, y, description) {
        browser.perform(() => comment(description));
        browser.execute(
            prepStringFuncForExecute(`function(x, y) {
      window.scrollTo(x, y);
    }`),
            [x, y],
            function (result) {}
    ...
    browser.scrollElement = function (
        selector,
        selectorType = 'CSS',
        x,
        y,
        ...
        var techDescription = stringFormat(
            "(Scrolling element at '%s' using '%s')",
            selector,
            selectorType
        );
        ...
        browser._elementPresent(selector, selectorType, null, timeout, () => {
            browser.assert.ok(
                false,
                stringFormat(
                    "FAILED: '%s' - Couldn't find element. %s",
                    description,
                    techDescription
                )
            );
        ...
        browser.execute(
            prepStringFuncForExecute(`function(selector, selectorType, x, y) {
    ...
    browser.scrollWindowToElement = function (
        selector,
        selectorType = 'CSS',
        description,
        timeout
        ...
        var techDescription = stringFormat(
            "(Scrolling window to el '%s' using '%s')",
            selector,
            selectorType
        );
        ...
        browser.execute(
            prepStringFuncForExecute(`function(selector, selectorType, value) {
    ...
    browser.click = function (
        ...
        browser._elementPresent(selector, selectorType, null, timeout, () => {
            browser.assert.ok(
                false,
                stringFormat(
                    "FAILED: '%s' - Couldn't find element to click. %s",
                    description,
                    techDescription
                )
            );
    ...
    browser.elStyleIs = function (
        ...
        function checkforStyle(selector, selectorType, style, value) {
            browser.execute(
                prepStringFuncForExecute(`function(selector, selectorType, style) {
        ${snptGetElement}
        var el = snptGetElement(selector, selectorType);
        return window.getComputedStyle(el, null).getPropertyValue(style);
      }`),
                [selector, selectorType, style],
                function (result) {
                    if (
    ...
    browser.inputValueAssert = function (
        selector,
        selectorType = 'CSS',
        value,
        description,
    ...
    browser.elementPresent = function (
        selector,
        selectorType = 'CSS',
        description,
        timeout
        ...
        var techDescription = stringFormat(
            "(Element exists' at '%s' using '%s')",
            selector,
            selectorType
        );

        browser._elementPresent(
            selector,
            selectorType,
            null,
            timeout,
            ...
            () => {
                browser.assert.ok(
                    true,
                    stringFormat("'%s' - %s", description, techDescription)
                );
            }
        );
    ...
    browser._elementPresent = function (
        selector,
        selectorType = 'CSS',
        description,
        timeout,
    ...
    browser.elementNotPresent = function (
        selector,
        selectorType = 'CSS',
        description,
        timeout
    ...
    browser.elTextIs = function (
        ...
        var techDescription = stringFormat(
            "(Assert text matches '%s' at '%s' using '%s')",
            assertText.toString(),
            selector,
            selectorType
        );
        ...
        function checkforText(selector, selectorType, assertText) {
            browser._getElText(selector, selectorType, function (elsText) {
                if (
                    assertText instanceof RegExp
                        ? assertText.test(elsText)
                        : assertText === elsText
                ) {
